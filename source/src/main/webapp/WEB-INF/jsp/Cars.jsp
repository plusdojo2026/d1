...
<body class="body">
	<form id=ca method="get" action="/d1/CarServlet">
	...
	<form method="GET" action="/d1/CarsServlet">
		...
		<c:forEach var="todo" items="${top1}">
			...
			<img alt="todoフォーム写真"
				src="${pageContext.request.contextPath}/img/${todo.outsidephoto}">
		</c:forEach>
		...
		<table class="table-view">
			...
			<c:forEach var="reserve" items="${history}">
				<tr>
					<td>${reserve.sdate.toString().replace('T', ' ')} ～
						${reserve.fdate.toString().replace('T', ' ')}</td>
