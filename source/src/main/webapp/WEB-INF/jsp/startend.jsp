	...
	<div class="info-a">
		<c:forEach var="syo" items="${reservelist}">
			<table class="table-info">
				<tr>
					<th>日程</th>
					<td>${syo.sdate.toString().replace('T', ' ')}～
						${syo.fdate.toString().replace('T', ' ')}</td>
				</tr>
				<tr>
					<th>使用車</th>
					<td>${syo.carname}</td>
	...
	<div class="button-group">
		<div class="btn-engine-start">
			<form action="/d1/StartendServlet" method="post">
				...
				<c:if test="${not empty starts}">
					<button type="submit" class="btn btn-engine-start-in" id="start">
						START<br> <span>ENGINE</span>
					</button>
				</c:if>
			</form>
		</div>
		<div class="btn-engine-start">
			<form action="/d1/StartendServlet" method="post">
				...
				<c:forEach var="re" items="${top1}">
					<input type="hidden" name="reservenumber" value="${re.reservenumber}">
				</c:forEach>
				<c:if test="${not empty top1}">
					<button type="submit" onclick="showQR()"
						${hasTodo ? '' : 'disabled'} class="btn btn-engine-start-in"
						id="ed">
						STOP<br> <span>ENGINE</span>
					</button>
				</c:if>
				<c:if test="${showQR}">
					<c:if test="${showQR}">
